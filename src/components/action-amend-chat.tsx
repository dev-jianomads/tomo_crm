...
import { getActionDrawerDraftPreview } from "@/components/action-drawer-panel";
import type { ActionItem } from "@/lib/mockData";

type ActionAmendChatProps = {
  entityKey: string;
  selection: DrawerSelection;
  action: ActionItem;
  assistance: TomoAssistance;
  onBack: () => void;
  /** After user confirms the amended outcome (e.g. send) */
  onFinalApprove: () => void;
...
export function ActionAmendChat({
      ...
      new DefaultChatTransport({
        ...
        body: {
          context: {
            ...
            page: "home" as const,
            selection,
            contextTitle: "Amend draft",
            assistanceContext: { ...assistance, executionChips: undefined },
          },
        },
      }),
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="action-amend-chat">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-1 py-3 text-sm">
        <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5">
          <p className="text-[11px] font-medium uppercase tracking-wide text-gray-500">Current draft</p>
          {preview.draftBody ? (
            <p className="whitespace-pre-line text-sm leading-relaxed text-gray-800">{preview.draftBody}</p>
          ) : (
      ...
      <div className="shrink-0 space-y-2 border-t border-gray-200 px-1 py-2">
        ...
        >
          <input
            ...
            placeholder="Tell Tomo how to amend…"
