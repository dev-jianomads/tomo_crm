...
export function WorkflowActivityLogAccordion({
  ...
  return (
    <div className="border-t border-gray-200 bg-white px-4 py-2" data-testid="workflow-activity-log-accordion">
      <details className="group">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 py-2 text-[11px] font-semibold uppercase tracking-wide text-gray-500 [&::-webkit-details-marker]:hidden">
        ...
        <p className="pb-2 text-[10px] leading-snug text-gray-500">
        ...
        <div className="overflow-x-auto pb-2">
          <table className="w-full min-w-[520px] border-collapse text-left text-[11px]">
            ...
            <tbody>
              {entries.map((e) => (
                <tr key={e.id} className="border-b border-gray-100 text-gray-800 last:border-0">
                  <td className="max-w-[120px] py-2 pr-2 align-top text-gray-700">{e.triggeredAt}</td>
                  <td className="max-w-[140px] py-2 pr-2 align-top text-gray-600">{e.listLabel}</td>
                  ...
                  <td className="py-2 text-right tabular-nums text-amber-900/90">{e.lpsSuppressed}</td>
                </tr>
              ))}
            </tbody>
          </table>
