  ...
  if (surface === "drawer") {
    ...
    if (wizardStep === "trigger") {
      ...
      lines.push(
        ...
        ``,
        `Your job is to **read the GP's context and their action description**, then **summarise, reframe, and optimise** it into a clear **action creation prompt** — meta-instructions for a separate Draft-step LLM. You are **not** writing the outreach email on this step.`,
        ``,
        `Tools:`,
        `- **confirm_workflow_action_prompt** — call when the optimised prompt is ready and the GP agrees (or after one brief refinement).`,
        ``,
        `Conversation flow:`,
        `1. Read **context text** and **attachment names** from context. Briefly acknowledge what you understood (1–2 sentences).`,
        `2. Refine the GP's intent into an **Optimised prompt** — bullet-style instructions covering tone, structure, length, personalization rules, CTA, and how to use context/attachments.`,
        `3. Ask once: "Does this look right? Say **yes** to lock it in, or tell me what to change."`,
        `4. When they agree, call **confirm_workflow_action_prompt**, then tell them to click **Generate drafts** (the next wizard step).`,
        ``,
        `Strict output rules for this step:`,
        `- **Never** write the email itself — no "Subject:" line, salutation, sign-off, or finished outreach copy.`,
        `- **Never** preview what the email would say; only describe **how** the Draft step should write it.`,
        `- Ask at most **one** clarifying question before proposing the optimised prompt.`,
        `- Do not ask about the trigger or list again unless the user wants to change them.`,
        ``,
        `When calling **confirm_workflow_action_prompt**:`,
        `- **instruction** — the full optimised meta-prompt (NOT email copy)`,
        `- **action_description** — one short sentence for the process-flow node`,
        `- **action_kind** — usually \`send_email\``,
        ``,
        `After the tool call, remind them to click **Generate drafts** — do not say the email is already written.`,
        `Keep replies concise.`,
      );
    } else {
      lines.push(
  ...
  if (surface === "workflow_creator") {
    ...
    if (wizardStep === "trigger") {
      ...
      tools.confirm_workflow_action_prompt = tool({
        description:
          "Lock the optimised action creation prompt (meta-instructions only — not email copy). Call after the GP agrees; the Draft step LLM uses this to generate the cohort email.",
