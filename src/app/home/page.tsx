    ...
    return [
      {
        ...
        source: "Source: Today actions (follow-ups + CRM signals)",
        tomoGenerated: true,
      },
      {
        ...
        source: "Source: Today commitments + linked briefs",
        tomoGenerated: true,
      },
      {
        ...
        source: "Source: Relationship momentum bands on Today",
        tomoGenerated: true,
      },
      {
        ...
        source: "Source: In-progress actions + brief open loops",
        tomoGenerated: true,
      },
    ];
...
function DailyBriefDialog({
  ...
  open: boolean;
  onClose: () => void;
  blocks: { title: string; subtitle: string; items: string[]; note?: string; source: string; tomoGenerated: boolean }[];
}) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 p-4 sm:items-center" onClick={onClose}>
      <div
        className="w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-4 shadow-xl sm:max-w-md sm:p-5"
        ...
        <div className="mt-4 max-h-[68vh] space-y-3 overflow-y-auto pr-1">
          {blocks.map((block, idx) => (
            <section key={block.title} className={`rounded-xl border px-3 py-3 ${sectionTone[idx % sectionTone.length]}`}>
              ...
              {block.tomoGenerated ? (
                <div className="mt-2 rounded-md border tomo-ai-border bg-white px-2.5 py-2">
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] font-semibold accent-title">Current snapshot</p>
                    <TomoAiBadge label="Tomo insight" />
                  </div>
                  <p className="mt-1 text-xs tomo-ai-text">Generated by Tomo from: {block.source.replace(/^Source:\s*/i, "")}</p>
                </div>
              ) : (
                <p className="mt-1 text-[11px] text-gray-500">{block.source}</p>
              )}
            </section>
          ))}
        </div>
