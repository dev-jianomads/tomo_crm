...
type LpIntroRow = { id: string; fundLabel: string; status: string; detail: string };

const MOCK_INTROS_TO_LP: LpIntroRow[] = [
  {
    id: "intro-demo-1",
  ...
  const detailContent = (
    <div className="h-full overflow-auto p-4 text-sm">
      ...
      <p className="mt-1 text-sm text-gray-600">
        Status flow: Pending → Meeting booked → In diligence → Closed (mock data below).
      </p>
      <ul className="mt-4 space-y-2">
        {MOCK_INTROS_TO_LP.map((row) => (
          <li key={row.id} className="rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
